...
export class ProcessingHelper {
  ...
  private async processScreenshotsHelper(
    ...
    try {
      ...
      let problemInfo;
      let testType: "mcq" | "coding" = "coding";

      // --- Unified prompt for both MCQ and coding detection ---
      const unifiedPrompt = `You are an exam assistant. First, detect if the screenshots are for a coding problem or a multiple-choice question (MCQ) test. 
If MCQ, extract the following fields in JSON: { test_type: 'mcq', question, options: [A, B, C, ...], correct_answer, explanation }. 
If coding, extract: { test_type: 'coding', problem_statement, constraints, example_input, example_output }. 
Return ONLY the JSON, no extra text. Preferred coding language: ${language}.`;
      ...
      if (config.apiProvider === "openai") {
        ...
        const messages = [
          {
            ...
            content: "You are a coding challenge and MCQ interpreter.",
          },
          {
            role: "user" as const,
            content: [
              { type: "text" as const, text: unifiedPrompt },
        ...
        try {
          ...
          testType = problemInfo.test_type === "mcq" ? "mcq" : "coding";
        } catch (error) {
      ...
      if (testType === "mcq") {
        if (mainWindow) {
          mainWindow.webContents.send("processing-status", {
            message: "MCQ detected. Extracting answer...",
            progress: 40,
          });
        }
        // Store MCQ info in AppState
        ...
        if (mainWindow) {
          ...
          // MCQ result: just return the answer and explanation
          const mcqResult = {
            testType: "mcq",
            question: problemInfo.question,
            options: problemInfo.options,
            correct_answer: problemInfo.correct_answer,
        ...
        return { success: false, error: "Failed to process MCQ screenshots" };
      }

      // --- Coding Handling (default) ---
      if (mainWindow) {
        mainWindow.webContents.send("processing-status", {
          message:
      ...
      if (mainWindow) {
        ...
        if (solutionsResult.success) {
          ...
          mainWindow.webContents.send(
            this.deps.PROCESSING_EVENTS.SOLUTION_SUCCESS,
            { ...solutionsResult.data, testType: "coding" }
          );
          return {
            success: true,
            data: { ...solutionsResult.data, testType: "coding" },
          };
        } else {
          throw new Error(
