...
// MCQ Section for displaying MCQ results
const MCQSection = ({
  question,
  options,
  correctAnswer,
  ...
  // Try to match answer by letter or by value
  let answerIndex = -1;
  let answerText = correctAnswer;
  if (Array.isArray(options) && options.length > 0 && correctAnswer) {
    // Try letter match (A, B, ...)
    const letterIndex =
      correctAnswer.trim().length === 1 && /^[A-Z]$/i.test(correctAnswer.trim())
        ? correctAnswer.trim().toUpperCase().charCodeAt(0) - 65
        : -1;
    if (letterIndex >= 0 && letterIndex < options.length) {
      answerIndex = letterIndex;
      answerText = options[letterIndex];
    } else {
      // Try value match
      answerIndex = options.findIndex(
        (opt) => opt.trim().toLowerCase() === correctAnswer.trim().toLowerCase()
      );
      if (answerIndex >= 0) answerText = options[answerIndex];
    }
  }
  // If answerText is still empty or just whitespace, use correctAnswer as fallback
  const showFallbackAnswer = !answerText || !answerText.trim();
  return (
    <div className="space-y-4">
      <div>
        ...
        <div className="text-[13px] text-gray-100 mb-2">{question}</div>
            ...
            {options.map((opt, idx) => (
              <li
                ...
                className={`px-3 py-2 rounded-md border text-[13px] ${
                  idx === answerIndex
                    ? "bg-green-700/60 border-green-400 text-white font-semibold"
        ...
        {/* Always show the answer below the options for clarity */}
        <div className="mt-3 text-[13px] text-green-200">
          <strong>Correct Answer:</strong>{" "}
          {showFallbackAnswer ? correctAnswer : answerText}
        </div>
      </div>
      {explanation && explanation.trim() && (
        <div className="mt-4">
          <h3 className="text-[13px] font-medium text-white/80 mb-1">
            Explanation
          </h3>
          <div className="text-[13px] text-gray-100 bg-white/5 rounded-md p-3">
            {explanation}
          </div>
        </div>
      )}
  ...
  return (
    <>
      ...
      {!isResetting && queryClient.getQueryData(["new_solution"]) ? (
        ...
        <div ref={contentRef} className="relative no-cursor">
          <div className="space-y-3 px-4 py-3">
            ...
            <SolutionCommands
              ...
              isProcessing={
                !problemStatementData || (!solutionData && !mcqData)
              }
